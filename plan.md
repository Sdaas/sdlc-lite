...
| #31 (skeleton) | v1 · step 2 | **DONE** | receipt schema (`analyzer/receipt.py`) rendered always, all-`UNKNOWN` verdicts; R3 effort-extraction + R4 `guard_decision` plumbing landed; effort policy widened to WARN-both-directions; Sources block + `SAMPLE-RECEIPT.md`. **Close phase (step 5) now complete via #31b-iii below — #31 ready to close.** |
...
| #31b-ii (accept: isolation) | v1 · step 5b-ii | **DONE (accepted; live FAIL deferred to #34)** | Real run (`/tmp/run-1`, factorial, 2026-09-14). **Both** in-band injections (test-writer `cd handoff && cat 03-*.md` leak; implementer `sed -i … tests/`) were **self-policed — both gates refused**; analyzer confirmed the refusals against the real artifacts (content audit ✅ no leak; detective ✅ no test-write). **Key insight:** the run-log detective and the model's self-policing key on the *same literal signal*, so a cooperative model refuses exactly what the path-based detective catches — the **auditor (content, method-agnostic) is the layer that can't be self-policed**. The unrun-but-designed auditor-FAIL vector (plant a design-internal line into the allowed `04-test-plan.md`) is recorded for #34. FAIL-path logic already unit+host-proven; this run proved the **plumbing + PASS path + effort-deviation catch** end-to-end (full pipeline incl. both Opus reviewers — most of #31b-iii for free). Findings: `design/acceptance-31bii-isolation-findings.md`. **Follow-ups:** (#28/#35) `test-reviewer effort low` NOT honored at runtime (ran `high`, audit flagged ⚠️); dev-container fresh-volume needs `claude plugin install` (postStartCommand gap) |
| #31b-iii (accept: clean run) | v1 · step 5b-iii | **DONE** | Real clean run (roman-numeral fixture, 2026-09-18, `/workspaces/roman-numeral-run` via new `test-fixtures/python-starter/` mechanism). Full pipeline incl. both Opus reviewers, one CHANGES-REQUESTED loop (error-message mutation gap, 82.4%→94.1% kill rate after fix), committed `a96b004` on `feature/00-roman-numeral`. Per-agent trust receipt all-✅ (isolation, model, effort all matched pins). **Bug found + fixed live:** Gate 11's own report showed a false `❌ isolation VIOLATION(S)` — `analyzer/runlog.py`'s `_handoff_dir()` had no `abspath()`, so the conductor's relative `--workdir` misclassified 5 legitimate handoff-outbox writes as product-tree violations; fixed + regression-tested + verdict wording corrected (⚠️ attempted/blocked ≠ ❌ trust-voiding). Host suite 160 green. **Closes #31 and completes v1.** Four follow-up issues filed (not blocking, see v1.1 rows below): #37 (error-message-contract test-writer/reviewer self-checks), #38 (mutmut fixture config + guard shell-variable blind spot), #39 (conductor mid-session model downgrade, low-priority), #40 (conductor-run mutmut, complements #38) |
| #36 (revert Witt model-enforce) | v1 | **DONE** | Removed `guard.py::_dispatch_deny_reason` (Task/Agent dispatch audited, never model-enforced) + SKILL.md now dispatches pinned gates **bare** so the honored frontmatter pin governs (incl. dated reviewer pins); ADR-12 rewritten to shipped state, prose sites corrected, findings §7 closed. Host suite 159 green. Commits `bb1686a`/`14ce596`/`5401c90`. **Unblocked #31b-iii** (and thus #35) |
| #35 | v1 · release | **DONE** | Flipped `test-reviewer` (`low`→`medium`) and `code-reviewer` (`high`→`medium`) frontmatter, plus every prose site describing the spread (SKILL.md's canonical table + 2 inline mentions + Rules invariant; DG's matching table/example/invariant, now with a historical note on why the spread existed). `test_agentdefs.py`'s tripwire test (reads the real agent-defs) updated to match. Host suite 160 green. **v1 cluster complete — ready to merge `v1-trust-claim` → `main`.** |
| #37 | v1.1 · backlog | pending | error-path tests: bare `pytest.raises(T)` survives Gate 3+4, caught only at Gate 7. Findings + drafted (unapplied) fix: `design/acceptance-31biii-error-message-contract-findings.md` |
| #38 | v1.1 · backlog | pending | fixture mutmut setup: pre-configure `[tool.mutmut]` in fixture templates + fix guard's shell-variable write-target blind spot + empirically verify against src-layout before trusting it |
| #39 | v2 · backlog | pending | conductor mid-session model downgrade (Opus for interview/design, cheaper after) — low priority, savings may not justify the complexity |
| #40 | v1.1 · backlog | pending | conductor (not code-reviewer) runs `mutmut` before dispatching CODE-REVIEW — complements #38, generalizes beyond our fixtures to any target repo |
