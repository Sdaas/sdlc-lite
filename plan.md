...
| #31b-iii (accept: clean run) | v1 · step 5b-iii | pending | **Expensive, one clean session post weekly-reset.** The clean all-PASS run: full pipeline incl. BOTH Opus reviewers → all-✅ receipt, committed. Observes the #28 dev-spread end-to-end. **Closing this closes #31 and completes v1.** Prereq: dev container up (#36 landed — the Witt hook no longer forces the reviewers onto `opus-5`). **On a fresh `sdlc-lite-claude` volume, run `claude plugin install implement-feature@daas-plugins` first** — `postStartCommand` writes `enabledPlugins` but does not install (gap found in #31b-ii; #31b-ii's run already exercised most of the clean pipeline incl. both Opus reviewers, so #31b-iii mainly needs the committed all-✅ receipt) |
